...
class Simple_RNNDataHandler:
    ...
    def init_user_times(self):
        ...
        for k, v in self.testset.items():
            times = []
            gap = (self.testset[k][0][0][0]-self.trainset[k][-1][0][0])/3600
            times.append(gap)
            for session_index in range(1,len(v)):
                gap = (self.testset[k][session_index][0][0]-self.testset[k][session_index-1][0][0])/3600
                times.append(gap)
            self.user_test_times[k] = times

    def get_times(self):
        return self.user_train_times, self.user_test_times
    ...
    def get_next_batch(self, dataset, dataset_session_lengths, user_times):
        session_batch = []
        session_lengths = []
        sess_rep_batch = []
        sess_rep_lengths = []
        ...
        user_list = Simple_RNNDataHandler.get_N_highest_indexes(remaining_sessions, self.batch_size)
        if(len(user_list) == 0):
            return [],[],[],[],[],[],[]
        ...
        for user in user_list:
            ...
            sess_gaps.append(user_times[user][session_index])
        ...
        return x, y, session_lengths, sess_rep_batch, sess_rep_lengths, user_list, sess_gaps
