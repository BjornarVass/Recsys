...
from datahandler_daytime import RNNDataHandler
...
TIME_RESOLUTION = 500
DAY_TIME_RESOLUTION = 24*7+1 #each half-hour + one for no entry
TIME_HIDDEN = 20
DAY_TIME_HIDDEN = 20
USER_HIDDEN = 30
ALPHA = 1.0
...
log_name = "lstm_hiddens20_120_avg_reluhidden2"
...
if(use_hidden):
    INTER_HIDDEN = INTRA_HIDDEN+TIME_HIDDEN+DAY_TIME_HIDDEN+USER_HIDDEN
...
print("Day time res: " + str(DAY_TIME_RESOLUTION))
print("Hidden: " + str(INTRA_HIDDEN))
print("Time hidden: " + str(TIME_HIDDEN))
print("Daytime hidden" + str(DAY_TIME_HIDDEN))
print("ALPHA: " + str(ALPHA))
print("BETA: " + str(BETA))

#setting of seed
torch.manual_seed(2) #seed CPU
...
day_time_embed = Embed(DAY_TIME_RESOLUTION, DAY_TIME_HIDDEN, True)
if(USE_CUDA):
    day_time_embed = day_time_embed.cuda(GPU)
day_time_embed_optimizer = torch.optim.Adam(day_time_embed.parameters(), lr=lr)
...
def process_batch(xinput, targetvalues, session_reps, sess_time_reps, sess_day_time_reps, first_Y):
    ...
    sess_day_times = torch.LongTensor(sess_day_time_reps)
    sess_day_times = Variable(sess_day_times)
    ...
    if(USE_CUDA):
        ...
        sess_day_times = sess_day_times.cuda(GPU)
...
def train_on_batch(xinput, targetvalues, sl, session_reps, sr_sl, user_list, sess_time_reps, sess_day_time_reps, time_targets, first_Y, timestamps):
    ...
    day_time_embed_optimizer.zero_grad()
    ...
    X, Y, S, T, DT, First = process_batch(xinput, targetvalues, session_reps, sess_time_reps, sess_day_time_reps, first_Y)

    #get embedded times
    embedded_T = time_embed(T)
    embedded_DT = day_time_embed(DT)
    ...
    embedded_U = embedded_U.expand(embedded_U.size(0), embedded_DT.size(1), embedded_U.size(2))
    ...
    hidden_cat = inter_rnn(torch.cat((S, embedded_T, embedded_DT, embedded_U),2), inter_hidden, inter_context, rep_indicies)
    ...
    if(use_hidden):
        datahandler.store_user_session_representations(hidden_out.data[0], user_list, time_targets, timestamps)
    else:
        datahandler.store_user_session_representations(mean_X.data, user_list, time_targets, timestamps)
    ...
    if(train_all):
        ...
        day_time_embed_optimizer.step()
        user_embed_optimizer.step()
        inter_optimizer.step()

    return mean_first_loss.data[0], mean_loss.data[0], mean_time_loss.data[0], times

def predict_on_batch(xinput, targetvalues, sl, session_reps, sr_sl, user_list, sess_time_reps, sess_day_time_reps, time_targets, first_Y, time_error, timestamps):
    X, Y, S, T, DT, First = process_batch(xinput, targetvalues, session_reps, sess_time_reps, sess_day_time_reps, first_Y)
...
while epoch_nr < MAX_EPOCHS:
    ...
    while(len(xinput) > int(BATCHSIZE/2)): #Why is the stopping condition this?
        ...
        #training call
        batch_first_loss, batch_loss, batch_time_loss, times = train_on_batch(xinput, targetvalues, sl, session_reps, sr_sl, user_list, sess_time_reps, sess_day_time_reps, time_targets, first_predictions, timestamps)
    ...
    datahandler.reset_user_batch_data()
    xinput, targetvalues, sl, session_reps, sr_sl, user_list, sess_time_reps, sess_day_time_reps, time_targets, first_predictions, timestamps = datahandler.get_next_test_batch()
    batch_nr = 0
    ...
    while(len(xinput) > int(BATCHSIZE/2)):
        ...
        k_predictions = predict_on_batch(xinput, targetvalues, sl, session_reps, sr_sl, user_list, sess_time_reps, sess_day_time_reps, time_targets, first_predictions, time_error, timestamps)
