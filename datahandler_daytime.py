...
class RNNDataHandler:
    ...
    def init_user_times(self):
        ...
        for k, v in self.trainset.items():
            ...
            for session_index in range(1,len(v)):
                ...
                gap = gap if gap > self.min_time else 0
                #gap = gap if gap < self.max_time else -1
                times.append(gap)
    ...
    def reset_user_session_representations(self):
        ...
        self.user_day_time_representations = [None]*self.num_users
        ...
        for k, v in self.trainset.items():
            ...
            self.user_day_time_representations[k] = collections.deque(maxlen=self.MAX_SESSION_REPRESENTATIONS)
            self.user_day_time_representations[k].append(0)
    ...
    def get_next_batch(self, dataset, dataset_session_lengths, time_set):
        ...
        for user in user_list:
            ...
            sess_day_time = list(self.user_day_time_representations[user])
            if(srl < self.MAX_SESSION_REPRESENTATIONS):
                for i in range(self.MAX_SESSION_REPRESENTATIONS-srl):
                    ...
                    sess_time.append(0)
                    sess_day_time.append(0)
            sess_rep_batch.append(sess_rep)
            sess_time_batch.append(sess_time)
            sess_day_time_batch.append(sess_day_time)
            timestamp_index = dataset_session_lengths[user][session_index]//2 - 1
            timestamps.append(dataset[user][session_index][timestamp_index][0])
        ...
        return x, y, session_lengths, sess_rep_batch, sess_rep_lengths, user_list, sess_time_batch, sess_day_time_batch, target_times, first_predictions, timestamps
    ...
    def store_user_session_representations(self, sessions_representations, user_list, target_times, timestamps):
        for i in range(len(user_list)):
            user = user_list[i]
            session_representation = list(sessions_representations[i])
            target_time = float(target_times[i])
            if(target_time < 0):
                target_time = 1/self.scale
            ...
            day_time_rep = time_obj.weekday()*24+time_obj.hour+1
            ...
            if(num_reps == 0):
                ...
                self.user_day_time_representations[user].pop()
            ...
            self.user_day_time_representations[user].append(day_time_rep)
            self.num_user_session_representations[user] = min(self.MAX_SESSION_REPRESENTATIONS, num_reps+1)
